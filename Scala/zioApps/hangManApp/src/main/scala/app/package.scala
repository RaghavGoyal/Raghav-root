package object app {
  // `sealed abstract` ensures that `copy` and `apply` methods (generated due to case keyword)
  // are not created for the class.
  // This is required to ensure that there is no way the `Name` object can be created / manipulated
  // such that name is empty.
  // If private keyword is put before class name, the class name can not be used in return types.
  // Putting Private after class name allows the name to be accessible; still keeping the class private.
  ...
  object Guess{
    def make(string: String):Option[Guess] ={
      Some(string.toList).collect{
  ...
  object State{
    def initial(name: Name, word: Word) = new State(name, Set.empty, word) {}
  }
